...
def run_model(size, num_bees=1, num_flowers=20, envType='countryside', visualize=False, NumSeason=1, seasonLength=100, iterations=10):

    beesim = BeeSim(size, num_bees, num_flowers, envType, visualize, NumSeason, seasonLength)
    ...
    return init_positions, positions #vor_init, vor, clustering_coefficients
...
def plot_clustering(size, num_bees=1, num_flowers=20, envType='countryside', visualize=False, NumSeason=1, seasonLength=100, iterations=10):
   ...
   # vor_init, vor, clustering_coefficients = run_model(size, num_bees, num_flowers, envType, visualize, NumSeason, seasonLength, iterations)
    ...
    init_positions, positions = run_model(size, num_bees, num_flowers, envType, visualize, NumSeason, seasonLength, iterations)
    ...
    if len(positions)>1:
        axes[1].scatter(positions[:,0], positions[:,1], marker='.')
    else:
        print("no final flowers")
        print(positions)
...
plot_clustering(size=1000, num_bees=numStartingBees, num_flowers=numStartingFlowers, envType='countryside', visualize=None, NumSeason=NumSeason, seasonLength=seasonLength, iterations=iterations)